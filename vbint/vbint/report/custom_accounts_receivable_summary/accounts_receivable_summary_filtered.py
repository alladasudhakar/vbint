...
def execute(filters=None):
   ...
   return AccountsReceivableSummaryFiltered(filters).run(args)


class AccountsReceivableSummaryFiltered(ReceivablePayableReport):
   def run(self, args):
   ...
   def get_data(self, args):
      ...
      if self.filters.show_gl_balance:
         ...
         print("gl_balance_map = " + str(gl_balance_map))
      ...
      for party, party_dict in self.party_total.items():
         ...
         if self.filters.show_future_payments:
            row.remaining_balance = flt(
               row.outstanding) - flt(row.future_amount)
         self.data.append(row)

   def get_party_total(self, args):
      self.party_total = frappe._dict()
      date_string = self.filters.start_date
      date_format = "%Y-%m-%d"
      st_date = datetime.strptime(date_string, date_format).date()
      for d in self.receivables:
         print("d.posting_date = " + str(d.get('posting_date')))
         if d.get('posting_date') >= st_date:
   ...
   def set_party_details(self, row):
      ...
      if row.sales_person:
         self.party_total[row.party].sales_person.append(
   ...
   def get_columns(self):
      ...
      if self.party_naming_by == "Naming Series":
         self.add_column(
            label=_("Supplier Name") if self.account_type == "Payable" else _(
               "Customer Name"),
      ...
      if self.account_type == "Receivable":
         ...
         if self.filters.sales_partner:
            self.add_column(label=_("Sales Partner"),
                            fieldname="default_sales_partner", fieldtype="Data")
...
def get_gl_balance(report_date, company, account_type):
   if account_type == "Payable":
      balance_calc_fields = [
         "party", {"SUM": [{"SUB": ["credit", "debit"]}], "as": "balance"}]
   else:
      balance_calc_fields = [
         "party", {"SUM": [{"SUB": ["debit", "credit"]}], "as": "balance"}]
   return frappe._dict(
      frappe.db.get_all(
         "GL Entry",
         fields=balance_calc_fields,
         filters={"posting_date": ("<=", report_date),
                  "is_cancelled": 0, "company": company},
         group_by="party",
         as_list=1,
      )
   )
